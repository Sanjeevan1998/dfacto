...
import json
import time
from agents.tools import search_duckduckgo_news
...
def search_node(state: GraphState):
    """
    Searches for news headlines using DuckDuckGo News.
    """
    keywords = state["keywords"]
    print(f"--> Searching DuckDuckGo News for: {keywords}")

    results = search_duckduckgo_news(keywords, max_results=15)

    if not results:
        print("No results from DuckDuckGo News.")
        return {"search_results": "No search results found."}

    # Format as a readable text block for Gemini extraction
    raw = "\n\n".join([
        f"Title: {r.get('title', '')}\n"
    ...
    print(f"--> Found {len(results)} results from DuckDuckGo News.")
    return {"search_results": raw}
