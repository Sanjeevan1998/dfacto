...
from agents.tools import search_duckduckgo_news, search_duckduckgo_text
import time
...
def fan_out_node(state: GraphState):
    print("FactChecker -> Searching for evidence via DuckDuckGo...")
    claim = state.get("core_claim", state["transcript"])
    depth = state.get("depth", 0)
    source_url = state.get("source_url")
    existing_evidence = state.get("worker_results", [])

    # Two searches: general text + news, for broader evidence coverage
    text_results = search_duckduckgo_text(f"fact check {claim}", max_results=5)
    news_results = search_duckduckgo_news(claim, max_results=5)
    all_results = text_results + news_results

    # Exclude the source URL to avoid circular evidence
    if source_url:
        all_results = [r for r in all_results if r.get("url", r.get("href", "")) != source_url]

    if not all_results:
        return {"worker_results": existing_evidence, "depth": depth + 1}
    ...
    new_evidence = _extract_evidence_items(claim, raw_evidence)
